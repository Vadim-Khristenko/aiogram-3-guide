...
async def cmd_donate(
    ...
    # If this is the /donate NUMBER command,
    # then extract the number from the command text
    if command.command != "donate":
        amount = int(command.command.split("_")[1])
    # Otherwise, we try to parse user input
    else:
        # Check for number and its range
        if (
    ...
    # For payments in Telegram Stars, the list of prices
    ...
    await message.answer_invoice(
        ...
        provider_token="",
        # You can pass anything in the payload,
        # for example, id of what exactly is being purchased
        payload=f"{amount}_stars",
        # XTR is the code for Telegram Stars currency
        currency="XTR",
